...
@register_agent()
class FloatingPandaGripperPad(BaseAgent):
    uid = "floating_panda_gripper_pad"
    urdf_path = f"{PACKAGE_ASSET_DIR}/robots/panda/panda_pad_gripper.urdf"  # You can use f"{PACKAGE_ASSET_DIR}" to reference a urdf file in the mani_skill /assets package folder
    urdf_config = dict(
        _materials=dict(
    ...
    keyframes = dict(
        ...
        open_facing_down=Keyframe(
            qpos=[-0.12, 0, 0.1, 0, np.pi, 0, 0.00, 0.00], pose=sapien.Pose(p=[0, 0, 0.2])
        ),
    )
    root_joint_names = [
        "root_x_axis_joint",
