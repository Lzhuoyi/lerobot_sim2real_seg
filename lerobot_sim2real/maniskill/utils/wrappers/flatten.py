...
class FlattenRGBDSegmentationObservationWrapper(gym.ObservationWrapper):
    """
    Flattens the rgbd+segmentation mode observations into a dictionary with keys for "rgb", "segmentation", and "state"

    Args:
        rgb (bool): Whether to include rgb images in the observation
        depth (bool): Whether to include depth images in the observation
        segmentation (bool): Whether to include segmentation images in the observation
        state (bool): Whether to include state data in the observation
        sep_depth (bool): Whether to separate depth and rgb images in the observation. Default is True.

    Note that the returned observations will have "rgb", "depth", and/or "segmentation" keys depending on the flags,
    and will always have a "state" key if state=True.
    """

    def __init__(self, env, rgb=True, depth=True, segmentation=True, state=True, sep_depth=True) -> None:
        self.base_env: BaseEnv = env.unwrapped
        super().__init__(env)
        self.include_rgb = rgb
        self.include_depth = depth
