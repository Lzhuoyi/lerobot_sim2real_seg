...
def update_camera(sim_env):
    global camera_offset, fov_offset, target_offset, last_frame_time, help_message_printed
    ...
    # Move the target position of the camera
    if "i" in active_keys:
        target_offset[0] -= MOVEMENT_SPEED * delta_time  # Move forward
    if "k" in active_keys:
        target_offset[0] += MOVEMENT_SPEED * delta_time  # Move back
    if "l" in active_keys:
        target_offset[1] += MOVEMENT_SPEED * delta_time  # Move right
    if "j" in active_keys:
        target_offset[1] -= MOVEMENT_SPEED * delta_time  # Move left
    if "o" in active_keys:     
        target_offset[2] += MOVEMENT_SPEED * delta_time  # Move up
    if "u" in active_keys:
        target_offset[2] -= MOVEMENT_SPEED * delta_time  # Move down

    # update camera position and fov
    pos = sim_env.unwrapped.base_camera_settings["pos"] + camera_offset
    target = sim_env.unwrapped.base_camera_settings["target"] + target_offset
    ...
    if len(active_keys) > 0:
        ...
        print("press: (i), (j) to move target in x, (k), (l) to move target in y, (o), (u) to move target in z")
...
def main(args: Args):
    ...
    while True:
        ...
        update_camera(sim_env)        # Redraw the plot
        fig.canvas.draw()
        fig.show()
